	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : MODIFICATION DE SUIVI DES MALADES </small> 
							 ...
							 ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><?php
													echo "<a href='ModifTourSalles.php?id=".$row['IdTour']."' title='Cliquez ici pour modifier les données du suivi'><span class='fa fa-edit'></span> Modifier le suivi</a>";	
												?>
											</td>                                           
                                        </tr>                                      
                                    </tbody>                                      
				...
				<div class="panel panel-primary">
					<div class="panel-body">
						...
						<div class="col-lg-12">
							...
							<center><h3> DONNEES DU SUIVI EFFECTUE</h3></center>
								<br/>
						</div>
						<div class="col-lg-4">
							...
							<div class="form-group"> 								
								...
								<input type="text" class="form-control" name="DateTour"  value="<?php echo $DateTour; ?>" placeholder="Date de suivi (jj-mm-aaaa)" required >
                            </div>
							<div class="form-group"> 								
								<label>Heure de suivi </label>	
								<input type="text" class="form-control" name="HeureTour"  value="<?php echo $row['HeureTour']; ?>" placeholder="Heure de suivi (hh:mm)" required >
                            </div>
							
							<label>Plainte du Malade</label>
                            <div class="form-group input-group"> 								                  
								<textarea class="form-control" name="Plainte" placeholder="Plainte donnée par le malade" style="width:380px" ><?php echo $row['Plainte']; ?></textarea>
                            </div>	
							<label>Prescription faite de Médicaments</label>
                            <div class="form-group input-group"> 								                  
								<textarea class="form-control" name="Prescription" placeholder="Précisez les médicaments prescrits et le dosage" style="width:380px" required ><?php echo $row['Prescription']; ?></textarea>
                            </div>							
                        </div>
						...
						<label>Les signes vitaux</label>
                            <div class="form-group input-group"> 	
								...
								<input type="text" class="form-control" name="Ta" value="<?php echo $row['Ta']; ?>" placeholder="Tension artérière" required >
                            </div>
							<div class="form-group input-group"> 	
								<span class="input-group-addon">PLS</span>                                           
								<input type="text" class="form-control" name="Pouls" value="<?php echo $row['Pouls']; ?>" placeholder="Pouls" required >
                            </div>
							<div class="form-group input-group"> 	
								<span class="input-group-addon">Poids</span>                                           
								<input type="text" class="form-control" name="Poids" value="<?php echo $row['Poids']; ?>" placeholder="Poids en Kilogramme" required >
                            </div>
							<div class="form-group input-group"> 	
								<span class="input-group-addon">FR</span>                                           
								<input type="text" class="form-control" name="FR" value="<?php echo $row['Fr']; ?>" placeholder="Fréquence Respiratoire" required >
                            </div>
							<div class="form-group input-group"> 	
								<span class="input-group-addon">FC</span>                                           
								<input type="text" class="form-control" name="FC" value="<?php echo $row['Fc']; ?>" placeholder="Fréquence Cardiaque" required >
                            </div>
						...
						<div class="col-lg-4">
                            ...
                            <div class="form-group input-group"> 	
								<span class="input-group-addon"><span class="glyphicon glyphicon-home"></span></span> </span>                                           
								<input type="text" class="form-control" name="Evolution" value="<?php echo $row['Evolution']; ?>" placeholder="Exemple: BIEN, ASSEZ BIEN, MAUVAIS, ..." required >
                            </div>
							<label>Observation faite</label>
                            <div class="form-group input-group"> 								                  
								<textarea class="form-control" name="Observation" placeholder="Parlez en bref de l'observation constatée" style="width:380px" required ><?php echo $row['Observation']; ?></textarea>
                            </div>
							
							<label>Médecin accompagnant</label>
                            <div class="form-group input-group"> 	
								<span class="input-group-addon"><span class="fa fa-male"></span></span> </span>                                           
								<input type="text" class="form-control" name="Accompagnant" value="<?php echo $row['Accompagnant']; ?>" placeholder="Nom du Médecin accompagnant" required >
                            </div>
						...
						<div class="col-lg-12">
								<center><button type="submit" class="btn btn-success" name="ModifTour"><span class="glyphicon glyphicon-edit"></span> Enregistrer les modifications </button></center>
                        </div>										
					</div>					
				</div>										
			</form>
...
<?php
	if(isset($_POST['ModifTour'])){
		
		$IdTour=$_POST['IdTour'];
		$Idauto_Patient=$_POST['Idauto_Patient'];
		$DateTour=new DateTime($_POST['DateTour']);
		$DateTour=Date_Format($DateTour,'Y-m-d');
		$HeureTour=$_POST['HeureTour'];
		...
		$MotPasse=$_POST['MotPasse'];
			...
			if(mysql_num_rows($rqt)>0){
				$req="UPDATE TourSalles SET DateTour='".$DateTour."', HeureTour='".$HeureTour."', Plainte='".$Plainte."', Prescription='".$Prescription."',
					Evolution='".$Evolution."', Observation='".$Observation."', Ta='".$Ta."', Pouls='".$Pouls."', Poids='".$Poids."', 
					Fr='".$FR."', Fc='".$FC."', Accompagnant='".$Accompagnant."' WHERE IdTour='".$IdTour."'";
				$result=mysql_query($req) or die(mysql_error());
					if (!$result){
						...
						echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-ok'></span> La modifcation du suivi du malade ".$_POST['Noms']." a été effectuée avec succès </div></div>";
					}
			}
			else{
				echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
