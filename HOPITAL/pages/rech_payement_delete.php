	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : SUPPRESSION DE PAYEMENT</small> 
				  <img src="IMA/error.png" width="100px" height="80px"/> 
             </h1>
                    <div class="panel panel-red">
                        <div class="panel-heading">
                           RECHERCHE DU PAIEMENT A SUPPRIMER
                        </div>
							 ...
							 ?>				
											<tbody>
												<tr>
													...
													<td><?php echo "<a href='rech_payement_delete.php?id=".$row['IdPaie']."' title='Cliquez ici pour afficher les détails du paiement'><span class='fa fa-list'></span> Afficher</a>";	?>
													</td>                                           
												</tr>                                      
											</tbody> 
							<?php
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						...
						<label>Monant Payé</label>
                            <div class="form-group input-group"> 
								...
								<input type="text" class="form-control" name="MontantPaie" value="<?php echo $row['MontantPaie']; ?>" readonly >
                            </div>
						...
						<div class="col-lg-3">							
							...
							<div class="form-group input-group">						                                          
								<textarea class="form-control" name="Observation" readonly><?php echo $row['Observation']; ?></textarea>
							</div>						
                        </div>
                       						
                        <div class="col-lg-12">						
							<center><button type="submit" class="btn btn-danger" name="Supprimer" onclick="return confirm('Etes-vous certain de supprimer définitivement ce payement?');"><span class="glyphicon glyphicon-trash"></span> Supprimer le payement</button></center>
						</div>					
					</form>
		...
		$Noms=$_POST['Noms'];
			...
			$rqt=mysql_query("SELECT * FROM Utilisateurs WHERE MotPasse LIKE BINARY '".$MotPasse."' AND (Fonction='MD' OR Fonction='AG')");
				if(mysql_num_rows($rqt)>0){				
					$req=mysql_query("DELETE FROM Payements WHERE IdPaie='".$IdPaie."'")or die(mysql_error());
						if (!$req){
							echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
							...
							echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
									<span class='glyphicon glyphicon-ok'></span> Le payement a été supprimé avec succès. </div></div>";
						}				
				}
				else{
					echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
