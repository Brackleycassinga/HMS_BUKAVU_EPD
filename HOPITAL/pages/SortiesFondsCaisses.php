	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : DECAISSEMENT </small> 
				  <img src="IMA/im20.jpg" width="120px" height="80px"/>
             </h1>
                    <div class="panel panel-red">
                        <div class="panel-heading">
                            CONSTATATION DE SORTIE (DECAISSEMENT) DES FONDS
                        </div>
                                ...
                                <div class="col-lg-6">														
                                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data" method="POST">
										...
										<div class="form-group"> 								
										 	<label>Date de décaissement à la caisse </label>	
										<table>                                          
										...
										<label>Montant décaissé en caisse (dollars)</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-usd"></span></span>
                                            <input type="text" class="form-control" name="MontantSortie" placeholder="Exemple : 12000" required >
                                        </div>
										
										<label>Motif de décaissement de fonds</label>
                                        <div class="form-group input-group"> 
									...
									<label>Agent ayant retiré l'argent</label>
                                        <div class="form-group input-group ">
										...
										<div class="form-group">
											<label>Document justificatif (Pièce jointe)</label>											 
                                            <input type="file"  name="Image" class="form-control" required />												
                                        </div>
										<label>Bénéficiaire</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-user"></span>$</span>
                                            <input type="text" class="form-control" name="Beneficiaire" placeholder="Noms complets" required >
                                        </div>
										</div>
										<div class="col-lg-12">
									<center><button type="submit" class="btn btn-success" name="Enregistrer"><span class="glyphicon glyphicon-share"></span> Enregistrer le décaissement </button></center>			
								</div>											 
						</form>
		 </div>
        ...
        if (is_uploaded_file($_FILES["Image"]["tmp_name"])) {
            if (rename($_FILES["Image"]["tmp_name"],
				...
				$rechSld=mysql_query("SELECT * FROM CAISSES");
						...
						$AncSld=mysql_fetch_row($rechSld);
						$Nvlsld=$AncSld[1]; 
						$Nvlsld2=$Nvlsld - $MontantSortie;
					 if($Nvlsld2 > 0){ 
						$requete = mysql_query("INSERT INTO SortiesFonds values('\N','$DateSortie','$MontantSortie','$Beneficiaire','$Motif','$Image','$IdAgent','$IdUtilisateur','$DateSortie','$IdUtilisateur')");
					 if($requete){
						 $sld=mysql_query("UPDATE CAISSES SET SoldeCaisse='$Nvlsld2', DateJour='$DateSortie'");						 					
