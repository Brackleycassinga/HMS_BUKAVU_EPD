	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : MODIFICATION DES EXAMENS </small> 
				 <img src="IMA/upcube.jpg" width="120px" height="80px" />
             </h1>
                    <div class="panel panel-green">
                        <div class="panel-heading">
                            RECHERCHE ET MODIFICATION DES EXAMENS
                        </div>
							 ...
							 ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><?php echo "<a href='ModifExamens.php?id=".$row['IdExamen']."' title='Modifier examen'><span class='glyphicon glyphicon-edit'></span> Modifier</a>";?></td>                                           
                                        </tr>                                      
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						<div class="col-lg-6">
                            ...
                            <div class="form-group input-group"> 
								...
								<input type="text" class="form-control" name="DesignExamen" value="<?php echo $row['DesignExamen']; ?>" required >
                            </div>
							<label>Prix Prévu</label>
                            <div class="form-group input-group"> 		
								<span class="input-group-addon"><span class="fa fa-dollar"></span></span> </span>                                           
								<input type="text" class="form-control" name="PrixPrevu" value="<?php echo $row['PrixPrevu']; ?>" required >
                            </div>
...
<?php
	if(isset($_POST['Modifier'])){
		...
		$DesignExamen=$_POST['DesignExamen'];	
		$PrixPrevu=$_POST['PrixPrevu'];	
			require_once("BDD/connect.php");
			
			$req=mysql_query("UPDATE Examens SET DesignExamen='".$DesignExamen."', PrixPrevu='".$PrixPrevu."', IdCategorie='".$IdCategorie."' WHERE Examens.IdExamen='".$IdExamen."'")or die(mysql_error());
				
					if (!$req){
						echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-remove'></span>Echec de modification de l'examen. vérifiez les données saisies<br>".mysql_error()."</div></div>";
					}
					else{ 
						echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-ok'></span> L'examen ".$DesignExamen." a été modifiée correctement </div></div>";
					}
			}
?>
