...
<script type="text/javascript">
	...
	function CountDown(){
		...
		if(counter > 0){
			...
			location.href = "ModifResultatExamens.php";
		}
	}
			...
			<div class="col-lg-16">
				...
				<div class="panel panel-green">
					<div class="panel-heading">
						MODIFICATION RESULTATS DES EXAMENS ET/OU TESTS TROUVES AU LABORATOIRE
					</div>
					<div class="panel-body">
						<div class="row">                                
							<?php
								if(isset($_GET['ModifResulat'])){
									$message="";
									...
									if(mysql_num_rows($rqt)>0){
										$edit=mysql_query("UPDATE ResultExamens SET IdPrescription='".$IdPrescription."' WHERE IdResultat='".$IdResultat."'");
											if($edit){
												
												$NbreRes=$_GET['NbreRes']; // Nbre de Resultat à modifier dans la base de données
												
												$j = "";											
													for($i=$NbreRes;$i>=1;$i--){ // Ceci permet de modifier à partir du premier champ, car ils sont nommés en décroissant.												
														if(isset($_GET["Resultat".$i])){
															$j = $i." ---";
															mysql_query("UPDATE ResultExamensCompose SET ResultatPropre='".$_GET["Resultat".$i]."' WHERE IdResultat='".$IdResultat."' AND IdExamen='".$_GET["IdExamen".$i]."'");
														}
													}
												$message = "<div class='col-lg-12'><div class='alert alert-success alert-dismissable'>
															...
															<span class='glyphicon glyphicon-ok'></span> &nbsp; Modification du Résultat des examens <strong>N° ".$IdResultat."</strong> a été effectué avec succès</center></div></div>";
											}
											else{
