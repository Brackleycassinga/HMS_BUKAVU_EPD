<?php 
	session_start();
	if(isset($_SESSION['IdUtilisateur'])&& isset($_SESSION['Login'])&& isset($_SESSION['MotPasse'])&& isset($_SESSION['Fonction'])&& ($_SESSION['Fonction']=='MEDECIN' OR $_SESSION['Fonction']=='MD')){
?>
<!DOCTYPE html>
<html lang="fr">
...
<script type="text/javascript">
	...
	function CountDown(){
		...
		if(counter > 0){
			...
			location.href = "rech_malade_prescription.php";
		}
	}
						...
						<div class="row">                                
							<?php
								if(isset($_GET['EnregPrescription'])){
									...
									$Nbre = $_GET['Nbre'];
									$save=mysql_query("INSERT INTO PrescExamens VALUES ('','".$DatePrescription."','".$Idauto_Patient."','".$IdUtilisateur."')");
										if($save){
											$select=mysql_query("SELECT max(IdPrescription) FROM PrescExamens WHERE Idauto_Patient='".$Idauto_Patient."'");
											$row=mysql_fetch_array($select);
											$IdPresc=$row[0];
												$j = "";
												for ($i=1;$i<=$Nbre;$i++){
													if(isset($_GET["IdExamen".$i])){
														$j = $i." ---";
														mysql_query("INSERT INTO PrescExamensCompose VALUES ('".$IdPresc."','".$_GET["IdExamen".$i]."')");
													}
												}
											$message = "<div class='col-lg-12'><div class='alert alert-success alert-dismissable'>
														...
														<span class='glyphicon glyphicon-ok'></span> &nbsp; Enregistrement de la Prescription des examens a été effectué avec succès</center></div></div>";
										}
										else{
