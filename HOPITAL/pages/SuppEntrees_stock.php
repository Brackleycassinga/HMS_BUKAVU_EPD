							 ...
							 ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><?php echo "<a href='SuppEntrees_stock.php?id=".$row['IdEntree']."' title='Supprimer le stockage'><span class='glyphicon glyphicon-list'></span> Afficher</a>";?></td>                                           
                                        </tr>                                      
								...
								<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">                               
									<div class="col-lg-6"> 
                                        ...
                                        <div class="form-group input-group">
                                            ...
                                            <input type="text" class="form-control" name="NumFacture" value="<?php echo $row['NumFacture'];?>" placeholder="Numéro de la facture" readonly >
                                        </div>
										<label>Numéro du lot</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-list-alt"></span></span>
                                            <input type="text" class="form-control" name="NumLot" value="<?php echo $row['NumLot'];?>" placeholder="Numéro du lot" readonly >
                                        </div>
										<label>Quantité entrée en stock</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-list-alt"></span></span>                                           
                                            <input type="text" class="form-control" name="QteEntree" value="<?php echo $row['QteEntree'];?>" readonly >
                                        </div>
                                    </div>
									<div class="col-lg-6">
                                        ...
                                        <div class="form-group input-group">
                                            ...
                                            <input type="text" class="form-control" name="PrixUnit" value="<?php echo $row['PrixUnit'];?>" readonly >
                                        </div>
										<label>Date de fabrication</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="fa fa-calendar"></span></span>
                                            <input type="text" class="form-control" name="DateFabrication" value="<?php echo $row['DateFabrication'];?>" readonly>
                                        </div>
										<label>Date de Péremption</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="fa fa-calendar"></span></span>
                                            <input type="text" class="form-control" name="DateExpiration" value="<?php echo $row['DateExpiration'];?>" readonly>
                                        </div>
										<label>Provenance</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="fa fa-home"></span></span>
                                            <input type="text" class="form-control" name="Provenance" value="<?php echo $row['Provenance'];?>" readonly>
                                        </div>
									...
									<div class="col-lg-12">
                                            <center><button type="submit" class="btn btn-danger" onclick="return confirm('Voulez-vous vraiment supprimer cette entrée en stock?');" name="Supprimer"><span class="glyphicon glyphicon-trash"></span> Supprimer le stockage</button></center>                                   									
									</div>
								</form>
	...
	if(isset($_POST['Supprimer'])){
		...
		$MotPasse=$_POST['MotPasse'];
			...
			if(mysql_num_rows($rqt)>0){
				...
				$Nvl=$rech['StockExistant'] - $QteEntree;
				if($Nvl>0){
					$req="DELETE FROM Entreestocks WHERE IdEntree='".$IdEntree."'";			
					...
					if (!$result){
						...
						echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-ok'></span> &nbsp; Suppression de l'entrée en stock effectuée avec succès. </div></div>";
					}
				}
				else{
					echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
							<span class='glyphicon glyphicon-remove'></span>Impossible de supprimer, car le stock deviendra négatif</div></div>";
				}
			}
			else{
				echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
