...
<script type="text/javascript">
	...
	function CountDown(){
		...
		if(counter > 0){
			...
			location.href = "ModifPrescriptionMedicaments.php";
		}
	}
		...
		<div class="row">
			<div class="col-lg-16">
				<h1 class="page-header" style="color:rgb(90, 100, 211);">
					 Gestion des Malades en ligne <small>: MODIFICATION DES PRESCRIPTION DES MEDICAMENTS</small> 
					  <img src="IMA/stockmed.png" width="180px" height="80px"/>
				 </h1>
				<div class="panel panel-green">
					<div class="panel-heading">
						MODIFICATION DES PRESCRIPTIONS DES EXAMENS ET/OU TESTS 
					</div>
					<div class="panel-body">
						<div class="row">                                
							<?php
								if(isset($_GET['ModifPrescription'])){
									$message="";
									...
									if(mysql_num_rows($rqt)>0){
										$res=mysql_query("SELECT * FROM SortieMedMalades WHERE IdPrescMed='".$IdPrescMed."'");
										if(mysql_num_rows($res)==0){
											$del=mysql_query("DELETE FROM PrescMedicamentsCompose WHERE IdPrescMed='".$IdPrescMed."'");									
												if($del){											
														$j = "";
														for ($i=1;$i<=$NbreMed;$i++){
															if(isset($_GET["IdMedicament".$i])){
																$j = $i." ---";
																mysql_query("INSERT INTO PrescMedicamentsCompose VALUES ('".$IdPrescMed."','".$_GET["IdMedicament".$i]."','".$_GET["Qte".$i]."','".$_GET["Consommation".$i]."')");
															}
														}
													$message = "<div class='col-lg-12'><div class='alert alert-success alert-dismissable'>
																...
																<span class='glyphicon glyphicon-ok'></span> &nbsp; Modification de la Prescription <strong> N° '".$IdPrescMed."'</strong> a été effectué avec succès</center></div></div>";
												}
												else{
											...
											$message = "<div class='col-lg-12'><div class='alert alert-danger alert-dismissable'>
														<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
														<center><img src='IMA/error.png'/><br><br>
														<span class='glyphicon glyphicon-alert'></span>Impossible de modifier, car cette prescription a déjà de livraison !! </center></div></div>";
