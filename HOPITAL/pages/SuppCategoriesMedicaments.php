		...
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : SUPPRESSION DES CATEGORIES </small> 
				  <img src="IMA/catmed.png" width="100px" height="80px"/><img src="IMA/11.jpg" width="100px" height="60px"/>
             </h1>
                    <div class="panel panel-red">
                        <div class="panel-heading">
                            RECHERCHE ET SUPPRESSIONS DE CATEGORIES DES MEDICAMENTS
                        </div>
							 ...
							 ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><?php echo "<a href='SuppCategoriesMedicaments.php?id=".$row['IdCategorieMed']."' title='Cliquez ici pour afficher le détail'><span class='glyphicon glyphicon-list'></span> Afficher</a>";?></td>                                           
                                        </tr>                                      
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						...
						<label>Designation de la catégorie</label>
                            <div class="form-group input-group"> 
								...
								<input type="text" class="form-control" name="DesignCategorieMed" value="<?php echo $row['DesignCategorieMed']; ?>" disabled >
                            </div>
                        </div>
                       						
                        <div class="col-lg-12">						
							<center><button type="submit" class="btn btn-danger" name="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer définitivement cette catégorie des médicaments?');"><span class="glyphicon glyphicon-trash"></span> Supprimer la catégorie</button></center>
						</div>					
					</form>
...
<?php
	if(isset($_POST['Supprimer'])){
		...
		$IdCategorieMed=$_POST['IdCategorieMed'];			
			require_once("BDD/connect.php");
			$sel=mysql_query("SELECT * FROM Medicaments WHERE IdCategorieMed='".$IdCategorieMed."'");
			if(mysql_num_rows($sel)){
				echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
						<span class='glyphicon glyphicon-remove'></span>Impossible de supprimer cette catégorie, car elle est liée aux médicaments dans la base de données.</div></div>";
			}
			else{
				$req=mysql_query("DELETE FROM CategoriesMed WHERE CategoriesMed.IdCategorieMed='".$IdCategorieMed."'");
