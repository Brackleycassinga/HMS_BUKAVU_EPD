	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : FORMES GALENIQUES </small> 
				  <img src="IMA/catmed.png" width="120px" height="100px"/>
             </h1>
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            AJOUT D'UNE NOUVELLE FORME GALENIQUE
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6">
                                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
										...
										<label>Désignation de la forme galenique</label>
                                        <div class="form-group input-group">
                                            <span class="input-group-addon"><span class="glyphicon glyphicon-list-alt"></span></span>
                                            <input type="text" class="form-control" name="Designation" placeholder="Exemple: INJECTABLE" required >
                                        </div>									
...
<?php
	if(isset($_POST['Enregistrer'])){
		...
		$Designation=$_POST['Designation'];
			...
			$search=mysql_query("SELECT * FROM CategoriesMed WHERE CategoriesMed.DesignCategorieMed='".$Designation."'");
			
			if (mysql_num_rows($search)>0){
				$kap=mysql_fetch_array($search);
				$nom=$kap['DesignCategorieMed'];
			...
			$req="INSERT INTO CategoriesMed VALUES('', '".$Designation."')";
				$result=mysql_query($req) or die(mysql_error());
					if (!$result){
						echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-remove'></span>Impossible d'enregistrer la catégorie, verifiez vos données. <br>".mysql_error()."</div></div>";
					}
					else{ 
						echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-ok'></span> &nbsp; Enregistrement effectué avec succès <br> Nouvelle catégorie des médicaments ajoutée à la base de données </div></div>";
					}
			}
		}
?>
