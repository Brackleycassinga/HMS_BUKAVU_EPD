	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : SUPPRESSION DE FACTURATION</small> 
				  <img src="IMA/im38.jpg" width="120px" height="80px"/><img src="IMA/11.jpg" width="120px" height="80px"/>
             </h1>
                    <div class="panel panel-red">
                        <div class="panel-heading">
                           RECHERCHE DE LA FACTURATION A SUPPRIMER
                        </div>
                            ...
                            <div class="row">                                
                                <div class="col-lg-6">
                                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
										...
										<label>Le Code ou Nom du Malade facturé</label>
									...
									if (mysql_num_rows($sqlSearch)>0){										
										echo"<div class='col-lg-12'>                       
												<div class='panel-body'>
													...
													<center><font style='font-weight:bold; color:red'>VOICI QUELQUE(S) PROPOSITION(S) TROUVEES POUR [ \" $Mot \"]</font></center>
														<br/>
													<table class='table'>
							 ...
							 ?>				
											<tbody>
												<tr>
													...
													<td><?php echo "<a href='rech_facturation_delete.php?id=".$row['IdFacturation']."' title='Cliquez ici pour afficher les détails de la facture '><span class='fa fa-list'></span> Visualisez</a>";	?>
													</td>                                           
												</tr>                                      
											</tbody> 
							<?php
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						...
						<label>Monant Facturé</label>
                            <div class="form-group input-group"> 
								...
								<input type="text" class="form-control" name="MontantFacture" value="<?php echo $row['MontantFacture']; ?>" readonly >
                            </div>
                        ...
                        <div class="col-lg-12">						
							<center><button type="submit" class="btn btn-danger" name="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer définitivement cette facturation?');"><span class="glyphicon glyphicon-trash"></span> Supprimer la facturation</button></center>
						</div>					
					</form>
	...
	if(isset($_POST['Supprimer'])){
		...
		$Noms=$_POST['Noms'];
			...
			if(mysql_num_rows($rqt)>0){
				...
				$sql=mysql_query("SELECT * FROM Payements WHERE IdFacturation='".$IdFacturation."'") or die(mysql_error());
				if(mysql_num_rows($sql)>0){
					echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-remove'></span> Impossible de supprimer cette facturation, car il y a de paiements pour celle-ci. Veuillez d'abord supprimer ses payements</div></div>";
				}
				else{
					$req=mysql_query("DELETE FROM Facturations WHERE Facturations.IdFacturation='".$IdFacturation."'")or die(mysql_error());
						
							if (!$req){
								echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
										<span class='glyphicon glyphicon-remove'></span>Echec de suppression de la facture. <br> ".mysql_error()."</div></div>";
							}
							else{ 
								echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
										<span class='glyphicon glyphicon-ok'></span> La facture du malade ".$Noms." a été supprimée correctement.</div></div>";
							}
				}
			}
			else{
