				  ...
				  <img src="IMA/11.jpg" width="100px" height="60px"/>
                    ...
                    <div class="panel panel-danger">
                        <div class="panel-heading">
                            RECHERCHE ET SUPPRESSIONS DES SERVICES
                        </div>
							 ...
							 ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><?php echo "<a href='SuppServices.php?id=".$row['CodeService']."' title='Supprimer la section'><span class='glyphicon glyphicon-list'></span> Afficher</a>";?></td>                                           
                                        </tr>                                      
							...
							<?php
										...
										echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
												<span class='glyphicon glyphicon-remove'></span>
													Aucune suggestion trouvée pour le mot <strong>[ $Mot ]</strong>, vérifiez-le et réessayer!!</div></div>";
									}
								}
							?>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						...
						<label>Designation du service</label>
                            <div class="form-group input-group"> 
								...
								<input type="text" class="form-control" name="DesignService" value="<?php echo $row['DesignService']; ?>" disabled >
                            </div>
                        </div>
                       						
                        <div class="col-lg-12">						
							<center><button type="submit" class="btn btn-danger" name="Supprimer" onclick="return confirm('Etes-vouos certains de vouloir supprimer définitivement ce service?');"><span class="glyphicon glyphicon-trash"></span> Supprimer le service</button></center>
						</div>					
					</form>
...
<?php
	if(isset($_POST['Supprimer'])){
		...
		$CodeService=$_POST['CodeService'];			
			...
			$req=mysql_query("DELETE FROM Services WHERE Services.CodeService='$CodeService'");
				
					if (!$req){
						echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-remove'></span>Echec de Suppression du service, car il est lié à d'autres enregistrements dans la base de données.</div></div>";
					}
					else{ 
						echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-ok'></span> Le service a été Supprimée correctement </div></div>";
					}
			}
?>
