	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                 Gestion des Malades en ligne <small>  : CATEGORIE DES MEDICAMENTS</small> 
				 <img src="IMA/catmed.png" width="120px" height="100px" /><img src="IMA/upcube.jpg" width="120px" height="100px" />
             </h1>
                    <div class="panel panel-green">
                        <div class="panel-heading">
                            RECHERCHE ET MODIFICATION DES CATEGORIES DES MEDICAMENTS
                        </div>
                            ...
                            <div class="row">                                
                                <div class="col-lg-6">
                                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
										...
										<label>L'appellation de la catégorie à rechercher</label>
							 ...
							 <?php
								if(isset($_POST['Rechercher'])){
									...
									$sqlSearch=mysql_query("SELECT * FROM CategoriesMed WHERE CategoriesMed.DesignCategorieMed LIKE '%$Mot%'") or die(mysql_error());
							 ...
							 ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td><?php echo $row['IdCategorieMed'];?></td>
                                            <td><?php echo $row['DesignCategorieMed'];?></td>                                          
                                            <td><?php echo "<a href='ModifCategoriesMedicaments.php?id=".$row['IdCategorieMed']."' title='Cliquez ici pour modifier la catégorie'><span class='glyphicon glyphicon-edit'></span> Modifier</a>";?></td>                                           
                                        </tr>                                      
							...
							<?php
										...
										echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
												<span class='glyphicon glyphicon-remove'></span>
													Aucune suggestion trouvée dans les categories pour le mot [".$Mot."], vérifiez-le et réessayer!!</div></div>";
									}
								}
							?>
					...
					<?php 
						if(isset($_GET['id'])){
						require_once("BDD/connect.php");
						$sql=mysql_query("SELECT * FROM CategoriesMed WHERE IdCategorieMed='".$_GET['id']."'");
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
						...
						<label>Designation de la catégorie</label>
                            <div class="form-group input-group"> 
								<input type="hidden" name="IdCategorieMed" value="<?php echo $row['IdCategorieMed']; ?>"></tr>		
								<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span> </span>                                           
								<input type="text" class="form-control" name="DesignCategorieMed" value="<?php echo $row['DesignCategorieMed']; ?>" required >
                            </div>
                        </div>
...
<?php
	if(isset($_POST['Modifier'])){
		...
		$DesignCategorieMed=$_POST['DesignCategorieMed'];	
			require_once("BDD/connect.php");
			
			$req=mysql_query("UPDATE CategoriesMed SET DesignCategorieMed='".$DesignCategorieMed."' WHERE CategoriesMed.IdCategorieMed='".$IdCategorieMed."'")or die(mysql_error());
				
					if (!$req){
						echo "<div class='col-lg-6'><div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-remove'></span>Echec de modification de la catégorie, vérifiez les données saisies</div></div>";
					}
					else{ 
						echo "<div class='col-lg-6'><div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
								<span class='glyphicon glyphicon-ok'></span> La catégorie ".$DesignCategorieMed." a été modifiée correctement </div></div>";
					}
			}
?>
