	...
	<div class="row">
		<div class="col-lg-16">
			<h1 class="page-header" style="color:rgb(90, 100, 211);">
                  Gestion des Malades en ligne <small>: SUPPRESSION DE PRESC. DES MEDICAMENTS</small> 
				  <img src="IMA/stockmed.png" width="180px" height="80px"/>
             </h1>
                    <div class="panel panel-red">
                        <div class="panel-heading">
                           RECHERCHE DE LA PRESCRIPTION DES MEDICAMENTS A SUPPRIMER
                        </div>
								...
								<div class="col-lg-6">
									<div class="panel panel-red">
										<div class="panel-body">
											...
											<p align="justify"><strong>Vous pouvez seulement supprimer les prescriptions qui n'ont pas encore été livrés par le pharmacien.
											Mais, si vous voulez supprimer une prescription ayant déjà des livraisons à la pharmacie; il va falloir d'abord annuler ses livraisons.</strong></p>
										</div>												
									</div>
								</div>								
                            </div>
												...
												<tr>
													...
													<td><?php $search=mysql_query("SELECT * FROM SortieMedMalades WHERE SortieMedMalades.IdPrescMed='".$row['IdPrescMed']."'");
														if(mysql_num_rows($search)==0)
															echo "<a href='SuppPrescriptionMedicaments.php?id=".$row['IdPrescMed']."' title='Cliquez ici pour afficher les détails'><span class='glyphicon glyphicon-list'></span> Afficher détails </a>";
														else
					...
					?>
						...
						<form action="supp_prescription_medicaments.php" method="GET" >
								...
								?>							
										<label>Medicament N° <?php echo $i;?></label>
											<div class="form-group input-group"> 	
												...
												<select class="form-control" style="width:400px;" name="IdMedicament<?php echo $i; ?>"  readonly >
													<option value="<?php echo $ligne['IdMedicament'] ?>"><?php echo $ligne['DesignMedicament']; ?></option>
													<?php
													require_once("BDD/Connect.php");
													$sel=mysql_query("SELECT * FROM Medicaments WHERE IdMedicament !='".$ligne['IdMedicament']."'");
													if(mysql_num_rows($sel)>0){
														while($row=mysql_fetch_array($sel)){
															echo "<option value='".$row['IdMedicament']."'>".$row['DesignMedicament']."</option>";
														}
													}
													?>											
												</select>
												<span class="input-group-addon">Qté <span class="glyphicon glyphicon-hand-right"></span></span>
												<input type="text"  class="form-control" name="Qte<?php echo $i; ?>" value="<?php echo $ligne['QuantiteDemande']; ?>" readonly >
											</div>
								<?php 
									}
								?>
											<center><button type="submit" class="btn btn-danger" onclick="return confirm('Voulez-vous vraiment supprimer définitivement cette préscription ?');" name="SuppPrescription"><span class="glyphicon glyphicon-trash"></span> Supprimer la préscription</button></center>
									</div>
